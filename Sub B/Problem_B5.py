...
class myCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs={}):
            if logs.get('val_mae') is not None and logs.get('val_mae') < 0.2:
...
def solution_B5():
    ...
    #trainings
    time_train= time_stepme[0:split_time]
    ...
    # YOUR CODE HERE ini model compile
    model.compile(loss='mae', optimizer='adam', metrics=['mae'])
    
    # YOUR CODE HERE ini model fit\
